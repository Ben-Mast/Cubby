import { deserializeModel, type Voxel, type VoxelData, type VoxelSize } from '../voxel/model'
import { DEFAULT_ROOM_DIMENSIONS, VOXEL_UNIT, type RoomDimensions } from './config'

export type RoomRotation = 0 | 90 | 180 | 270
export interface PlacedFurniture {
  id: string; home_id: string; furniture_id: string; x: number; y: number; z: number; rotation: RoomRotation; updated_at?: string
}
...
export function reconstructRoomModel(data: VoxelData): RoomModel {
  ...
  return {
    ...
    maxY: Math.max(...voxels.map(voxel => voxel.y)) + 1,
    maxZ: Math.max(...voxels.map(voxel => voxel.z)) + 1,
  }
}

export function roomTransform(placement: PlacedFurniture, model: RoomModel, dimensions: RoomDimensions = DEFAULT_ROOM_DIMENSIONS) {
  ...
  return {
    position: [-dimensions.width * VOXEL_UNIT / 2 + (placement.x - minX) * VOXEL_UNIT,
      (placement.y - model.minY) * VOXEL_UNIT, -dimensions.depth * VOXEL_UNIT / 2 + (placement.z - minZ) * VOXEL_UNIT] as [number, number, number],
